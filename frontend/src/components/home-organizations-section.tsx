...
const HomeOrganizationsSection = () => {
  return (
    <section className="relative m-auto flex max-w-screen-xl flex-col items-center justify-center gap-4">
      ...
      <p className="w-3/5 min-w-56 text-center mb-6">
        Are you an Organization or company that hosts student Events and want to
        publish them on our platform? <br></br> Feel free to contact us and we look
      ...
      <div className="flex flex-col justify-around gap-4 sm:flex-row">
        {cardContents.map((card, index) => (
          <Card
            key={index}
            className="flex max-w-[400px] flex-col rounded-md border border-muted shadow-sm hover:shadow-lg sm:w-1/3 lg:w-1/4"
          >
            <CardHeader>
              <card.icon size={64} className="mb-2" />
            </CardHeader>
            <CardContent>
