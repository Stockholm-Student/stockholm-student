    ...
    <>
      {!!user && (
        <div style={{display: 'flex', width: "100%", justifyContent: "space-around"}}>
          <button style={{border: "1px solid black", padding: 3}} onClick={() => fetchApi()}>fetch public</button>
          <button style={{border: "1px solid black", padding: 3}} onClick={() => fetchApi(true)}>fetch secure</button>
        </div>
      )}
      <p>{msgText}</p>
    </>
